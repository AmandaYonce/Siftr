...
def scan_folder(folder: Path, conn: Connection, recursive: bool = True) -> int:
    ...
    for path in images:
        ...
        if not (row and row["mtime"] == stat.st_mtime and row["size"] == stat.st_size):
    ...
    def safe_process(path: Path) -> ProcessedImage | None:
        try:
            ...
            logger.warning("Skipping unreadable image: %s", path, exc_info=True)
            return None
        ...
        for (path, stat), info in zip(to_process, results):
            ...
            conn.execute(
                ...
                ON CONFLICT(path) DO UPDATE SET
                    ...
                    sharpness = excluded.sharpness, thumb_path = excluded.thumb_path
                """,
                (
                    str(path),
                    stat.st_mtime,
